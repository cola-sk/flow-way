...
async function callTencentDrivingAPI(
  ...
  return (data.result.routes as any[]).map((route) => ({
    ...
    steps: (route.steps as any[] || []).map((step: any) => ({
      ...
      polylineIdxStart: (step.polyline_idx && step.polyline_idx.length > 0) ? step.polyline_idx[0] : 0,
      polylineIdxEnd: (step.polyline_idx && step.polyline_idx.length > 1) ? step.polyline_idx[1] : 0,
      action: step.act,
